                ...
                <div className="max-w-6xl mx-auto space-y-8">
                    <header className="flex items-end justify-between">
                        <div>
                            ...
                            <p className="text-gray-400 text-sm">Token usage by users and models.</p>
                        </div>
                        <div className="flex gap-2">
                            {(["24h", "7d", "30d"] as const).map((r) => (
                                <button
                                    key={r}
                                    onClick={() => setRange(r)}
                                    className={`px-3 py-1.5 rounded-lg text-xs ${range === r ? "bg-white/10" : "bg-white/5"}`}
                                >
                                    {r}
                                </button>
                            ))}
                    ...
                    <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                            { label: "Input", value: data?.totalInputTokens || 0 },
                            { label: "Output", value: data?.totalOutputTokens || 0 },
                        ].map((item) => (
                            <div
                                key={item.label}
                            ...
                            >
                                ...
                                <p className="text-3xl font-bold mt-2">{formatTokens(item.value)}</p>
                            </div>
                        ))}
                    </section>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {(data?.topUsersByTokenUsage || []).map((u) => (
                                        <tr key={u.userId || u.username || u.fullname || Math.random()}>
                                            <td className="py-3">
                                                {u.fullname || u.username || u.userId || "—"}
                                            </td>
                                            <td className="py-3 text-right">{u.requestCount}</td>
                    ...
                    <section className="p-6 rounded-xl bg-white/2 border border-white/5">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {(data?.topModelsByTokenUsage || []).map((m) => (
                                        <tr key={m.model}>
                                            <td className="py-3">{m.model}</td>
                                            <td className="py-3 text-right">{m.requestCount}</td>
                                            <td className="py-3 text-right">
