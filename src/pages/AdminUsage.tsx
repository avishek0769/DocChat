...
const AdminUsage = () => {
    ...
    return (
        <div className="min-h-screen bg-[#0b0b0f] text-gray-50 flex font-sans"><Sidebar /><main className="flex-1 p-8 lg:p-12 overflow-y-auto w-full"><div className="max-w-6xl mx-auto space-y-8">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4">{[{label:"Input",value:data?.totals?.inputTokens||0},{label:"Output",value:data?.totals?.outputTokens||0}].map((item)=><div key={item.label} className="p-5 rounded-xl bg-white/2 border border-white/5"><p className="text-gray-400 text-sm">{item.label}</p><p className="text-3xl font-bold mt-2">{formatTokens(item.value)}</p></div>)}</section>
            <section className="p-6 rounded-xl bg-white/2 border border-white/5"><h2 className="text-lg font-semibold mb-4">Top Users</h2><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-gray-400 border-b border-white/5"><th className="text-left pb-3">User</th><th className="text-right pb-3">Total</th></tr></thead><tbody className="divide-y divide-white/5">{(data?.topUsers||[]).map((u)=><tr key={u.id}><td className="py-3">{u.fullname||u.username||u.email||u.id}</td><td className="py-3 text-right">{formatTokens(u.totalTokens||((u.inputTokens||0)+(u.outputTokens||0)))}</td></tr>)}</tbody></table></div></section>
            <section className="p-6 rounded-xl bg-white/2 border border-white/5"><h2 className="text-lg font-semibold mb-4">Top Models</h2><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-gray-400 border-b border-white/5"><th className="text-left pb-3">Model</th><th className="text-right pb-3">Input</th><th className="text-right pb-3">Output</th><th className="text-right pb-3">Total</th></tr></thead><tbody className="divide-y divide-white/5">{(data?.topModels||[]).map((m)=><tr key={m.model}><td className="py-3">{m.model}</td><td className="py-3 text-right">{formatTokens(m.totalInputTokens)}</td><td className="py-3 text-right">{formatTokens(m.totalOutputTokens)}</td><td className="py-3 text-right">{formatTokens(m.totalTokens)}</td></tr>)}</tbody></table></div></section>
        </div></main></div>
    );
};
